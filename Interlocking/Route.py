...
from Interlocking import RouteActivationSection
from Interlocking import SwitchAndPosition
from Interlocking import RouteEntry
...
class Route(TrackAsset):
	...
	def setLocksAutomatically(self, aLocksAutomatically : long):
		self.___locksAutomatically = aLocksAutomatically

	def getLocksAutomatically(self) -> long:
		return self.___locksAutomatically

	def setProcessingDelay(self, aProcessingDelay : duration):
		self.___processingDelay = aProcessingDelay

	def getProcessingDelay(self) -> duration:
		return self.___processingDelay

	def setProceedAspectDelay(self, aProceedAspectDelay : duration):
		self.___proceedAspectDelay = aProceedAspectDelay

	def getProceedAspectDelay(self) -> duration:
		return self.___proceedAspectDelay

	def setSignalClosureDelay(self, aSignalClosureDelay : duration):
		self.___signalClosureDelay = aSignalClosureDelay

	def getSignalClosureDelay(self) -> duration:
		return self.___signalClosureDelay

	def setApproachReleaseDelay(self, aApproachReleaseDelay : duration):
		self.___approachReleaseDelay = aApproachReleaseDelay

	def getApproachReleaseDelay(self) -> duration:
